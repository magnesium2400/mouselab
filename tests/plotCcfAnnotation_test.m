...
V = getAllenTemplate('ara_nissl',50); % example volumetric data
figure;  plotAllenSlice('dim', 's', 'slice', 30, 'resolution', 100);
hold on; plotCcfAnnotation(V, 'dim', 's', 'slice', 59, 'resolution', 50);
colormap('sky');
...
figuremax;
for ii = ["coronal","transverse","sagittal"]
    uit = uitab("Title",ii); ax = axes('Parent', uit);
    p = plotAllenSlice('resolution', res, 'slice', 0.55, 'dim', extractBefore(ii,2));
